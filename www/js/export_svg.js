...
function isAnalyticsEntrySettled(entry) {
    if (!entry || !entry.containerId) return false;
    var container = document.getElementById(entry.containerId);
...
function waitForAnalyticsEntries(entries, timeoutMs, callback, onProgress) {
    var list = Array.isArray(entries) ? entries.slice() : [];
    var maxWait = Math.max(0, Number(timeoutMs || 0));
    var started = Date.now();
    var previousReady = -1;
    ...
    function poll() {
        var readyCount = list.filter(isAnalyticsEntrySettled).length;
        if (readyCount !== previousReady && typeof onProgress === "function") {
            previousReady = readyCount;
            onProgress(readyCount, list.length);
        }
...
function getAnalyticsExportButton(mode) {
    return document.getElementById("btn_download_all_" + String(mode || "").toLowerCase() + "_analytics_svg");
}

function updateAnalyticsExportButton(mode, readyCount, totalCount) {
    var button = getAnalyticsExportButton(mode);
    if (!button || button.getAttribute("aria-busy") !== "true") return;
    var ready = Math.max(0, Number(readyCount || 0));
    var total = Math.max(0, Number(totalCount || 0));
    button.innerHTML = '<i class="fa fa-spinner fa-spin" aria-hidden="true"></i><span> Preparing ' +
        ready + "/" + total + "…</span>";
    button.title = "Preparing every available Analytics SVG for the ZIP";
}

function beginAnalyticsExportButton(mode, totalCount) {
    var button = getAnalyticsExportButton(mode);
    if (!button) return true;
    if (button.getAttribute("aria-busy") === "true") return false;
    clearTimeout(button._cgvAnalyticsExportRestoreTimer);
    button._cgvAnalyticsExportOriginalHtml = button.innerHTML;
    button._cgvAnalyticsExportOriginalTitle = button.title;
    button.disabled = true;
    button.setAttribute("aria-disabled", "true");
    button.setAttribute("aria-busy", "true");
    updateAnalyticsExportButton(mode, 0, totalCount);
    return true;
}

function finishAnalyticsExportButton(mode, exportedCount) {
    var button = getAnalyticsExportButton(mode);
    if (!button) return;
    var count = Math.max(0, Number(exportedCount || 0));
    button.innerHTML = count > 0
        ? '<i class="fa fa-check" aria-hidden="true"></i><span> Downloaded ' + count + " SVG" + (count === 1 ? "" : "s") + "</span>"
        : '<i class="fa fa-exclamation-circle" aria-hidden="true"></i><span> Nothing to export</span>';
    button.title = count > 0
        ? "The Analytics SVG ZIP was downloaded"
        : "No Analytics charts with sufficient data were available";
    button._cgvAnalyticsExportRestoreTimer = setTimeout(function () {
        button.innerHTML = button._cgvAnalyticsExportOriginalHtml || '<i class="fa fa-file-zipper" aria-hidden="true"></i><span> SVG ZIP</span>';
        button.title = button._cgvAnalyticsExportOriginalTitle || "";
...
function exportAnalyticsSVGs(mode) {
    ...
    if (!beginAnalyticsExportButton(key, entries.length)) return;
    ...
    setTimeout(function () {
        waitForAnalyticsEntries(namedEntries, ENTRY_TIMEOUT_MS, function () {
            ...
            exportSVGCollection(exportableEntries, zipName, {
                ...
                onComplete: function (count) {
                    finishAnalyticsExportButton(key, count);
                    if (originalTabValue) {
        ...
        }, function (readyCount, totalCount) {
            updateAnalyticsExportButton(key, readyCount, totalCount);
        });
    }, 250);
}
