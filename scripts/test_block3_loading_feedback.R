...
popup_markup <- sub('.*id = "app-status-popup",', '', ui_txt)
popup_markup <- sub('id = "app-work-indicator".*', '', popup_markup)
stopifnot(grepl('class = "app-status-popup-current-label"', popup_markup, fixed = TRUE))
stopifnot(!grepl('class = "app-dna-loader"', popup_markup, fixed = TRUE))
...
stopifnot(grepl("document.querySelector('.app-main')", activity_txt, fixed = TRUE))
stopifnot(grepl("main.getBoundingClientRect()", activity_txt, fixed = TRUE))
stopifnot(grepl("new window.ResizeObserver", activity_txt, fixed = TRUE))
stopifnot(grepl("new window.MutationObserver", activity_txt, fixed = TRUE))
stopifnot(grepl("shell.addEventListener('transitionend'", activity_txt, fixed = TRUE))
stopifnot(grepl("data-layout-placement", activity_txt, fixed = TRUE))
stopifnot(grepl("popup.classList.contains('open')", activity_txt, fixed = TRUE))
stopifnot(grepl("refreshLayout: scheduleIndicatorLayout", activity_txt, fixed = TRUE))
...
stopifnot(grepl("window.cgvActivity.refreshLayout()", popup_txt, fixed = TRUE))
...
stopifnot(grepl(".app-status-popup-current-label", app_css, fixed = TRUE))
