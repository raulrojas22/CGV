...
stopifnot(grepl('shiny::HTML("Complete &mdash; generate every available view (recommended)")', shared_domain_source, fixed = TRUE))
stopifnot(grepl('shiny::HTML("Fast &mdash; include only views already rendered")', shared_domain_source, fixed = TRUE))
stopifnot(grepl('choiceValues = c("complete", "fast")', shared_domain_source, fixed = TRUE))
...
stopifnot(grepl('versioned_asset_path("favicon2.ico?v=2")', shared_domain_source, fixed = TRUE))
stopifnot(grepl("`data-light-src` = light_logo_src", shared_domain_source, fixed = TRUE))
stopifnot(grepl("`data-dark-src` = dark_logo_src", shared_domain_source, fixed = TRUE))
